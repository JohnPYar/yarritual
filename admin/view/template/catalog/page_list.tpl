        ...
        <table class="list">
          <thead>
            <tr>
              ...
              <td class="left"><?php if ($sort == 'pd.name') { ?>
                ...
                <?php } ?></td>
              <td class="left"><?php if ($sort == 'p.article') { ?>
          ...
          <tbody>
            <tr class="filter">
              ...
              <td><input type="text" name="filter_article" value="<?php echo $filter_article; ?>" /></td>
            ...
            <tr>
              <td style="text-align: center;"><?php if ($page['selected']) { ?>
                ...
                <?php } ?></td>
              <td class="center"><img src="<?php echo $page['image']; ?>" alt="<?php echo $page['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /></td>
              <td class="left"><?php echo $page['name']; ?></td>
              <td class="left"><?php echo $page['article']; ?></td>
...
function filter() {
	url = 'index.php?route=catalog/page&token=<?php echo $token; ?>';
	...
	var filter_article = $('input[name=\'filter_article\']').attr('value');
...
$('input[name=\'filter_article\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/page/autocomplete&token=<?php echo $token; ?>&filter_article=' +  encodeURIComponent(request.term),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
	...
	select: function(event, ui) {
		$('input[name=\'filter_article\']').val(ui.item.label);
