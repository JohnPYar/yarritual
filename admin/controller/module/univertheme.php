...
class ControllerModuleuniverTheme extends Controller {
	...
	public function index() {   
		...
		$this->data['text_buy_view'] = $this->language->get('text_buy_view');
		$this->data['text_m_image'] = $this->language->get('text_m_image');
		...
		$this->data['text_topcontact'] = $this->language->get('text_topcontact');
		...
		$this->data['univer_categ_text'] = $this->language->get('univer_categ_text');
		...
		$this->data['menu_border_text'] = $this->language->get('menu_border_text');
		...
		$this->data['banner_slider_button'] = $this->language->get('banner_slider_button');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('univertheme', $this->request->post);		
		...
		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/univertheme', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/univertheme', 'token=' . $this->session->data['token'], 'SSL');
	...
	/////////////////////////////////////////////////////	
        ...
        $Variables_matt = array(
		  //GENERAL SETTINGS
		   ...
		   //TOP MENU
			...
			'button_bg_color','button_border_color', 'button_link_color',     'button_bg2_color', 'button_link2_color',
			...
			'news_data',
			'feed_text_color', 'feed_button_bg',
			
			'other_show1_link',
			'other_heading',
			'other_bg',
			'other_prod1_bg', 'other_prod2_bg',
			'other_banner_heading1', 
            'other_bannerslider1', 
			'slider_button', 'slider_button_text',
		...
		if (isset($this->request->post['univermod'])) {
			$this->data['mattData'] = $this->request->post['univermod'];
		} elseif ($this->config->get('univermod')) { 
			$this->data['mattData'] = $this->config->get('univermod');
		}
		$this->data['mattlink'] = array();
		if (isset($this->request->post['univerlink'])) {
			$this->data['mattlink'] = $this->request->post['univerlink'];
		} elseif ($this->config->get('univerlink')) { 
			$this->data['mattlink'] = $this->config->get('univerlink');
		}
        $this->data['mattlink2'] = array();
		if (isset($this->request->post['univerlinkheader'])) {
			$this->data['mattlink2'] = $this->request->post['univerlinkheader'];
		} elseif ($this->config->get('univerlinkheader')) { 
			$this->data['mattlink2'] = $this->config->get('univerlinkheader');
		}
  ...
  //Header icons
		if (isset($this->request->post['mattheader'])) {
			$mattheaders = $this->request->post['mattheader'];
		}elseif ($this->config->get('mattheader')) { 
			$mattheaders =  $this->config->get('mattheader');
		} else {
			$mattheaders = array();
		}
		
	
		foreach ($mattheaders as $mattheader) {
			if ($mattheader['image'] && file_exists(DIR_IMAGE . $mattheader['image'])) {
				$image = $mattheader['image'];
			} else {
				$image = false;
			}			
			
			$this->data['mattheaders'][] = array(
				'image'                    => $image,
				'thumb'                    => $this->model_tool_image->resize($image, 50, 50),
				'title'                    => $mattheader['title'] ,
				'href'                     => $mattheader['href'] ,
			);	
		} 
		...
		$this->template = 'module/univertheme.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/univertheme')) {
