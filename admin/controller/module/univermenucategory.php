...
class ControllerModuleunivermenucategory extends Controller {
	private $error = array(); 
	private $_name = 'univermenucategory';
	public function index() {   
	
		$this->load->language('module/univermenucategory');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('univermenucategory', $this->request->post);		
		...
		if (isset($this->error['numproduct'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/univermenucategory', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/univermenucategory', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['univermenucategory_tab'])) {
			$tabs = $this->request->post['univermenucategory_tab'];
		} elseif ($this->config->get('univermenucategory_tab')) { 
		...
		if (isset($this->request->post['univermenucategory_module'])) {
			$this->data['modules'] = $this->request->post['univermenucategory_module'];
		} elseif ($this->config->get('univermenucategory_module')) { 
			$this->data['modules'] = $this->config->get('univermenucategory_module');
		}	
		...
		$this->template = 'module/univermenucategory.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);		
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/univermenucategory')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['univermenucategory_module'])) {
			foreach ($this->request->post['univermenucategory_module'] as $key => $value) {
  ...
  if ($query->num_rows) {
   ...
   return 0;
	...
	public function install() 
	{
		...
		$univermenucategory = array('univermenucategory_tab' => array ( 
        0 => array ('category_id' => 20, 
		            'subcateg' => 1, 
					'column' => 1, 
					'showproducts' => 1,
					'image' => 'data/univer/m_ic1.png', 
					'title' => $array_title0),
			1 => array ('category_id' => 18, 
			            'subcateg' => 1,
						'column' => 1, 
						'image' => 'data/univer/m_ic2.png', 
						'title' => $array_title1)
		),'univermenucategory_module' => array ( 
			0 => array ('image_category_width' => 360, 
		...
		$this->model_setting_setting->editSetting('univermenucategory', $univermenucategory);		
	}	
}
?>