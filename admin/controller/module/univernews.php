...
class ControllerModuleunivernews extends Controller {
	...
	public function index() {
		...
		$this->model_catalog_univernews->checkunivernews();
	...
	public function insert() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
			$this->model_catalog_univernews->addunivernews($this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->redirect($this->url->link('module/univernews/listing', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	public function update() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
			$this->model_catalog_univernews->editunivernews($this->request->get['univernews_id'], $this->request->post);
	...
	public function delete() { 
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $univernews_id) {
				$this->model_catalog_univernews->deleteunivernews($univernews_id);
			}
	...
	private function getModule() { 
		...
		$this->data['entry_row'] = $this->language->get('entry_row');
		...
		if (isset($this->error['univernewspage_thumb'])) {
			$this->data['error_newspage_thumb'] = $this->error['univernewspage_thumb'];
		} else {
			$this->data['error_newspage_thumb'] = '';
		}
	
		if (isset($this->error['newspage_popup'])) {
			$this->data['error_newspage_popup'] = $this->error['univernewspage_popup'];
		} else {
		...
		$this->data['univernews'] = $this->url->link('module/univernews/listing', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post[$this->_name . '_univernewspage_addthis'])) {
			$this->data[$this->_name . '_univernewspage_addthis'] = $this->request->post[$this->_name . '_univernewspage_addthis'];
		} else {
			$this->data[$this->_name . '_univernewspage_addthis'] = $this->config->get($this->_name . '_univernewspage_addthis');
		}
		...
		$this->template = 'module/univernews.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getList() {
		...
		$this->data['breadcrumbs'][] = array(
			'href'      => $this->url->link('module/univernews/listing', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);
	
		$this->data['module'] = $this->url->link('module/univernews', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['insert'] = $this->url->link('module/univernews/insert', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('module/univernews/delete', 'token=' . $this->session->data['token'], 'SSL');
	
		$this->data['totalunivernews'] = $this->model_catalog_univernews->getTotalunivernews();
	
		$this->load->model('tool/image');
	
		$this->data['univernews'] = array();
	
		$results = $this->model_catalog_univernews->getunivernews();
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('module/univernews/update', 'token=' . $this->session->data['token'] . '&univernews_id=' . $result['univernews_id'], 'SSL')
			);
			...
			$this->data['univernews'][] = array(
				'univernews_id'     	=> $result['univernews_id'],
				'title'       		=> $result['title'],
				'image'      		=> $image,
				'date_added'  	=> date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'viewed'			=> $result['viewed'],
				'status'     		=> ($result['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled')),
				'selected'    	=> isset($this->request->post['selected']) && in_array($result['univernews_id'], $this->request->post['selected']),
				'action'      		=> $action
			);
		}
	
		$this->template = 'module/univernews/list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm() { 
		...
		if (!isset($this->request->get['univernews_id'])) {
			$this->data['action'] = $this->url->link('module/univernews/insert', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$this->data['action'] = $this->url->link('module/univernews/update', 'token=' . $this->session->data['token'] . '&univernews_id=' . $this->request->get['univernews_id'], 'SSL');
		}
	
		$this->data['cancel'] = $this->url->link('module/univernews/listing', 'token=' . $this->session->data['token'], 'SSL');
	
		if ((isset($this->request->get['univernews_id'])) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$univernews_info = $this->model_catalog_univernews->getunivernewsStory($this->request->get['univernews_id']);
		}
		$this->load->model('localisation/language');
	
		$this->data['languages'] = $this->model_localisation_language->getLanguages();
	
		if (isset($this->request->post['univernews_description'])) {
			$this->data['univernews_description'] = $this->request->post['univernews_description'];
		} elseif (isset($this->request->get['univernews_id'])) {
			$this->data['univernews_description'] = $this->model_catalog_univernews->getunivernewsDescriptions($this->request->get['univernews_id']);
		} else {
			$this->data['univernews_description'] = array();
		}
		
		if (isset($this->request->post['meta_keyword'])) {
			$this->data['meta_keyword'] = $this->request->post['meta_keyword'];
		} elseif (isset($this->request->get['univernews_id'])) {
			$this->data['meta_keyword'] = $this->model_catalog_univernews->getunivernewsDescriptions($this->request->get['univernews_id']);
		} else {
		...
		} elseif (isset($univernews_info['date_added'])) {
			$this->data['date_added'] = $univernews_info['date_added'];
		} else {
		...
		if (isset($this->request->post['univernews_store'])) {
			$this->data['univernews_store'] = $this->request->post['univernews_store'];
		} elseif (isset($univernews_info)) {
			$this->data['univernews_store'] = $this->model_catalog_univernews->getunivernewsStores($this->request->get['univernews_id']);
		} else {
			$this->data['univernews_store'] = array(0);
		}
	
		if (isset($this->request->post['keyword'])) {
			$this->data['keyword'] = $this->request->post['keyword'];
		} elseif (isset($univernews_info)) {
			$this->data['keyword'] = $univernews_info['keyword'];
		} else {
			$this->data['keyword'] = '';
		}
		...
		if (isset($this->request->post['status'])) {
			...
			$this->data['status'] = $univernews_info['status'];
		} else {
			$this->data['status'] = '';
		}
		...
		} elseif (!empty($univernews_info)) {
			$this->data['image'] = $univernews_info['image'];
		} else {
			$this->data['image'] = '';
		}
		...
		} elseif (!empty($univernews_info) && $univernews_info['image'] && file_exists(DIR_IMAGE . $univernews_info['image'])) {
			$this->data['thumb'] = $this->model_tool_image->resize($univernews_info['image'], 100, 100);
		} else {
			$this->data['thumb'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		}
	
		$this->template = 'module/univernews/form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/univernews')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
	
		if (!$this->request->post['univernews_headline_chars']) {
			$this->error['numchars'] = $this->language->get('error_numchars');
		}
	
		if (!$this->request->post['univernews_thumb_width'] || !$this->request->post['univernews_thumb_height']) {
			$this->error['univernewspage_thumb'] = $this->language->get('error_newspage_thumb');
		}
	
		if (!$this->request->post['univernews_popup_width'] || !$this->request->post['univernews_popup_height']) {
			$this->error['univernewspage_popup'] = $this->language->get('error_newspage_popup');
		}
	...
	private function validateForm() {
		...
		foreach ($this->request->post['univernews_description'] as $language_id => $value) {
	...
	public function install() { 
		//create univernews table
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "univernews`");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "univernews` (`univernews_id` int(11) NOT NULL auto_increment, `status` int(1) NOT NULL default '0', `image` VARCHAR(255) COLLATE utf8_general_ci default NULL, `image_size` int(1) NOT NULL default '0', `date_added` date default NULL, `viewed` int(5) NOT NULL DEFAULT '0', PRIMARY KEY (`univernews_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci");
		//create univernews description table
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "univernews_description`");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "univernews_description` (`univernews_id` int(11) NOT NULL default '0', `language_id` int(11) NOT NULL default '0', `title` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `meta_description` VARCHAR(255) COLLATE utf8_general_ci NOT NULL, `description` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `meta_keyword` varchar(255) COLLATE utf8_general_ci NOT NULL, PRIMARY KEY (`univernews_id`,`language_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci");
		//create univernews store table
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "univernews_to_store`");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "univernews_to_store` (`univernews_id` int(11) NOT NULL, `store_id` int(11) NOT NULL, PRIMARY KEY (`univernews_id`, `store_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci");
    }
	...
	public function uninstall() { 
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE `query` LIKE 'univernews_id=%'");
    }
}
?>