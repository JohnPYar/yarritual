...
class ControllerModuleuniverslider extends Controller {
	private $error = array(); 
	private $_name = 'universlider';
	public function index() {   
	
		$this->load->language('module/universlider');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('universlider', $this->request->post);
		...
		$this->data['item_button'] = $this->language->get('item_button');
		...
		if (isset($this->error['category_image'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/universlider', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/universlider', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['universlider_module'])) {
			$this->data['modules'] = $this->request->post['universlider_module'];
		} elseif ($this->config->get('universlider_module')) { 
			$this->data['modules'] = $this->config->get('universlider_module');
		}	
      ...
      'tabs'          => 
		...
		$this->template = 'module/universlider.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);		
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/universlider')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['universlider_module'])) {
			foreach ($this->request->post['universlider_module'] as $key => $value) {
