	...
	public function ajaxify(){
		if(isset($this->request->get['product_id']) && (int)$this->request->get['product_id'] > 0){
			...
			if ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) {
				$image = $this->model_tool_image->resize($result['image'], 46, 46);
                $popup = $this->model_tool_image->resize($result['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
			} else {
				$image = $this->model_tool_image->resize('no_image.jpg', 46, 46);
			...
			$this->data['products'][] = array(
                ...
                'popup'             => $popup,
				'frontend'          => HTTP_CATALOG . 'index.php?route=product/product' . '&product_id=' . $result['product_id'],
			);
		}
	...
	protected function validateSpecialForm(){
  	...
  	private function getList() {
		...
		if(!isset($_SESSION['current_rows'])){
			$_SESSION['current_rows'] = $this->config->get('config_admin_limit');
		}
		$this->data['current_rows'] = $_SESSION['current_rows'];
		$this->data['selected_language'] = isset($_SESSION['product_language'])?(int)$_SESSION['product_language']:(int)$this->config->get('config_language_id');
        ...
        $this->data['text_no_category'] = $this->language->get('text_no_category');
		...
		$this->data['frontend_price_gross'] = $this->language->get('frontend_price_gross');
		$this->data['frontend_price_net'] = $this->language->get('frontend_price_net');
		$this->data['rows_in_table'] = $this->language->get('rows_in_table');
		...
		$this->data['date_column_switch'] = $this->language->get('date_column_switch');
		...
		$this->data['sort_id'] = 'index.php?route=catalog/product_extra&token=' . $this->session->data['token'] . '&sort=p.product_id' . $url;
		...
		$this->data['sort_date'] = 'index.php?route=catalog/product_extra&token=' . $this->session->data['token'] . '&sort=p.date_added' . $url;
		...
		$pagination->limit = $_SESSION['current_rows'];//$this->config->get('config_admin_limit');
