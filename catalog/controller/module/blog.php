...
class ControllerModuleBlog extends Controller {
	protected function index() {
        ...
        $this->load->model('tool/image');
         ...
         if ($blog_info) {
             	if ($blog_info['image']) {
				$thumb = $this->model_tool_image->resize($blog_info['image'],1200,482, 1);
			} else {
				$thumb = '';
			}
			...
			foreach ($children as $child) {
				...
				$record_total = $this->model_catalog_record->getTotalRecords($data);
		         ...
		         if ($blog_child_info) {
						...
						$thumb_child = $this->model_tool_image->resize($blog_child_info['image'], 1084,475);
					} else {
						$thumb_child = '';
					}
			...
			$this->data['blogies'][] = array(
				...
				'color'		  => $blog['meta_keyword'],
				'description'  => utf8_substr(strip_tags(html_entity_decode($blog['description'], ENT_QUOTES, 'UTF-8')), 0, 350) . '',
				'thumb'		  => $thumb,
