...
class ControllerModule1000CategoriesAccordion extends Controller {
	...
	protected function loadCategories($parent_id, $current_path = '', $cid = '') {
		...
		if ($results) { 
			if ($parent_id == 0) {
				...
				$ret_string .= '<ul id="children_' . $parent_id . '" class="turboq1">'; 
			}
		}
		...
		foreach ($results as $result) {	
			...
			if ($result['top']=="0") {
				...
				$ret_string .= '<li class="cid' . $result['category_id'] . ' ' . $classactive . ' ' . $children1 . '">';
				if ($children) {
					$ret_string .= '<a class="havechild ' . $classactive . ' expand-categ" '. $displaya .' href="' . $this->url->link('product/category','path=' .  $new_path)  . '" category="'.$result['category_id'].'" path="'.$new_path.'">' . $result['name'] . '</a>';
				} else { 
					$ret_string .= '<a class="nochild ' . $classactive . '" '. $displaya .' href="' . $this->url->link('product/category','path=' .  $new_path)  . '">' . $result['name'] . '</a>';
				}			
	...
	public function ajax() {
		...
		foreach ($results as $result) {	
			...
			$ret_string .= '<li class="cid' . $result['category_id'] . '" category="'.$result['category_id'].' wwwww">';
