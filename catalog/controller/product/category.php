...
class ControllerProductCategory extends Controller {  
	public function index() { 
		...
		if ($category_info) {
			...
			$this->data['category_view'] = $category_info['category_view'];
			...
			foreach ($results as $result) {
				...
				$image = $this->model_tool_image->resize($result['image'], 281, 160);
				$this->data['categories'][] = array(
					'name'  => $result['name'] . ($this->config->get('config_product_count') ? ' (' . $product_total . ')' : ''),
					'description'  => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, 150) . '',
					'seo_h1'  => utf8_substr(strip_tags(html_entity_decode($result['seo_h1'], ENT_QUOTES, 'UTF-8')), 0, 150) . '',
