	...
	if (view == 'list') {
		...
		$('.product-grid > div').each(function(index, element) {
		    ...
		    <?php if  ((isset($product['quickview'])) && ($this->config->get('quick_view') == '1')) { ?>  
             html += ' <div class="quickviewbutton">' + $(element).find('.quickviewbutton').html() + '</div>';
            <?php } ?>
			 <?php if  ($this->config->get('buy_view') == '1') { ?>  
			html += '<div class="cart">' + $(element).find('.cart').html() + '</div>'; 
			<?php } ?>
			html += '</div>'; 
			html += '</div>';
		 ...
		 $(".owl-addimagecat").owlCarousel({ navigation : true, 
		  ...
		  $('.item > div').hover(function(){
           var $gallery = $(this).parents('.item');
	       $('.owl-addimagecat div a',$gallery).hover(function(){
		   $('.owl-addimagecat div',$gallery).removeClass('active');
		   $(this).parent().addClass('active');
		   $('.image img',$gallery).attr('src', $(this).attr('data-image'));});
	       $('.owl-addimagecat div:first',$gallery).addClass('active');
	    });		
